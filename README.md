...
| Cloze | Replaces only the active blank with `什么`, `what`, or `なに`; nested Clozes also read their full Concept/Descriptor path. |
...
| Cloze | 只把当前挖空替换为 `什么`、`what` 或 `なに`；嵌套 Cloze 还会先朗读完整的 Concept/Descriptor 路径。 |
